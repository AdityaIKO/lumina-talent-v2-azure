...
export default function Keuangan() {
  ...
  // KYC state
  const [kycStep, setKycStep] = useState(1); // 1: upload, 2: processing, 3: phone, 4: otp
  ...
  // Cleanup OTP state when KYC modal closes
  useEffect(() => {
    if (!showKycModal) resetOtp();
  }, [showKycModal]);
  ...
  const handleKycSendOtp = async () => {
    if (!kycPhone || kycPhone.replace(/\D/g, '').length < 9) {
      return toast('Masukkan nomor HP yang valid (min. 9 digit).', 'error');
    }
    setOtpSending(true);
    try {
      const normalized = await sendOtp(kycPhone);
      toast(`Kode OTP dikirimkan ke ${normalized}`, 'success');
      setKycStep(4);
    } catch (err) {
      console.error('[OTP]', err);
      toast(err?.message?.includes('TOO_SHORT') || err?.message?.includes('INVALID')
        ? 'Nomor HP tidak valid. Pastikan format benar (contoh: 08123456789).'
        : err?.message?.includes('quota') || err?.message?.includes('QUOTA')
          ? 'Batas pengiriman SMS tercapai. Coba beberapa saat lagi.'
          : 'Gagal mengirim OTP. Periksa nomor HP dan coba lagi.', 'error');
    } finally {
      setOtpSending(false);
    }
  };
  ...
  const handleKycVerify = async () => {
    ...
    try {
      ...
      await updateProfile({ kycVerified: true, kycVerifiedAt: new Date().toISOString(), phone: kycPhone });
      ...
      toast(err?.message?.includes('invalid-verification-code') || err?.message?.includes('INVALID_CODE')
        ? 'Kode OTP salah. Periksa SMS Anda dan coba lagi.'
        : 'Verifikasi gagal. Coba kirim ulang OTP.', 'error');
    } finally {
          ...
          <div className="modal" style={{ maxWidth: '520px' }}>
            ...
            <div className="modal-body">
              ...
              <div style={{ display: 'flex', gap: '6px', marginBottom: '24px', alignItems: 'center' }}>
                {['Unggah Dokumen', 'Proses OCR', 'Nomor HP', 'Verifikasi OTP'].map((label, idx) => {
                  const stepNum = idx + 1;
                  ...
                  return (
                    <React.Fragment key={label}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flex: 1 }}>
                        ...
                        <span style={{ fontSize: '0.72rem', color: active ? 'var(--text)' : 'var(--text-muted)', whiteSpace: 'nowrap' }}>{label}</span>
                      </div>
                      {idx < 3 && <div style={{ flex: 0, width: 16, height: 1, background: 'var(--border)' }} />}
                    </React.Fragment>
                  );
                })}
              </div>
                ...
                <>
                  <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                    ...
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: '6px' }}>
                      Kode OTP akan dikirimkan ke nomor ini untuk memverifikasi identitas Anda.
                    </p>
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onKeyDown={(e) => e.key === 'Enter' && handleKycSendOtp()}
                    />
                  </div>
                  <button
                    className="btn btn-primary w-full"
                    style={{ marginTop: '8px' }}
                    onClick={handleKycSendOtp}
                    disabled={otpSending}
                  >
                    {otpSending ? 'Mengirim OTP...' : 'Kirim Kode OTP'}
                  </button>
                </>
              )}
              ...
              {kycStep === 4 && (
                <>
                  <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                    ...
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', marginTop: '6px' }}>
                      Kode OTP 6 digit telah dikirimkan ke <strong>{kycPhone}</strong>.
                    </p>
                  </div>
                  ...
                  >
                    {otpVerifying ? 'Memverifikasi...' : 'Verifikasi & Lanjutkan'}
                  </button>
                  <button
                    className="btn btn-ghost w-full"
                    style={{ marginTop: '8px' }}
                    onClick={() => { setKycStep(3); setKycOtp(['', '', '', '', '', '']); }}
                    disabled={otpVerifying}
                  >
                    Kirim Ulang OTP
                  </button>

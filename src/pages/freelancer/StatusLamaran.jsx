...
import { getUserApplications } from '../../services/database';
...
export default function StatusLamaran() {
  ...
  useEffect(() => {
    if (!user) return;
    getUserApplications(user.uid || user.id)
          ...
          <div id="statusContent">
            {activeTab === 'applications' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {loading ? (
                  <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>Memuat lamaran...</div>
                ) : applications.length === 0 ? (
                  <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
                    <div style={{ fontSize: '2.5rem', marginBottom: '16px' }}>📭</div>
                    <h4 style={{ marginBottom: '8px' }}>Belum ada lamaran</h4>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
                      Lamaran yang Anda kirim akan muncul di sini.
                    </p>
                  </div>
                ) : applications.map(a => (
                  <div key={a.id} className="card">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                      <div>
                        <div style={{ fontWeight: 600, marginBottom: '4px' }}>{a.jobTitle || a.job}</div>
                        <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                          {a.company} · {new Date(a.appliedAt).toLocaleDateString('id-ID')}
                        </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                        ...
                        {a.match && <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Match {a.match}%</div>}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {activeTab === 'contracts' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {(user?.contracts || []).length === 0 && (
                  <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
                    <div style={{ fontSize: '2.5rem', marginBottom: '16px' }}>📋</div>
                    <h4 style={{ marginBottom: '8px' }}>Belum ada kontrak aktif</h4>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
                      Kontrak yang diterima oleh Employer akan muncul di sini.
                    </p>
                  </div>
                )}
            ...
            {activeTab === 'history' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {(user?.workHistory || []).length === 0 ? (
                  <div className="card" style={{ textAlign: 'center', padding: '48px 24px' }}>
                    <div style={{ fontSize: '2.5rem', marginBottom: '16px' }}>🏆</div>
                    <h4 style={{ marginBottom: '8px' }}>Belum ada riwayat pekerjaan</h4>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
                      Pekerjaan yang sudah selesai akan tersimpan di sini beserta ulasan dari klien.
                    </p>
                  </div>
